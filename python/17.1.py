...
class LinkedList(object):
    def __init__(self):
        self._begin = Node(-1, None)
        self._end = Node(-1, self._begin)
        self._begin._next = self._end

    def push_front(self, val):
        self._begin._next = Node(val, self._begin._next)

    def insert(self, iterator, val):
        iterator.ptr._next = Node(val, iterator.ptr._next)


class LinkedListIterator(object):
    def __init__(self, llist):
        self.llist = llist
        self.ptr = llist._begin._next

    def forward(self):
        self.ptr = self.ptr._next
        if self.ptr == self.llist._end:
            self.ptr = self.llist._begin._next


def main():
    inp = 303

    # Assume part 2 requires some form of constant time
    # insertion and model this part as a linked list

    llist = LinkedList()
    llist.push_front(0)

    iterator = LinkedListIterator(llist)
    for i in range(1, 2017 + 1):
        for _ in range(0, inp):
            iterator.forward()
        llist.insert(iterator, i)
        iterator.forward()

    iterator.forward()
    print iterator.ptr.val
